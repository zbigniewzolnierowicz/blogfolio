...
reset ? `radial-gradient(circle at 0% 0%, transparent, transparent 75%), var(--cardColorPrimary)` : `radial-gradient(circle at ${xPercentage}% ${yPercentage}%, rgba(255, 255, 255, 0.3), transparent 75%), var(--cardColorPrimary)`
...
function setRotationAndPercentage(ref, angle) {
    return function(event) {
        ...
        gsap.to(ref, {
            rotationX: rotX,
            rotationY: rotY,
            background: generateGradient(((xPercentage + 1) / 2) * 100, ((yPercentage + 1) / 2) * 100, angle === 0)
        });
    }
}
...
const PageCard = ({ main, chips, header, timeToRead, onClick, path }) => {
    ...
    return (
        <PostCardContainer
            onMouseMoveCapture={(event) => setRotationAndPercentage(ref.current, 7.5)(event)}
        ...
        >
            ...
            <main css={css`grid-area: main;`}>
                {main}
            </main>
            <div className="chips" role="list" css={css`
                grid-area: chips;
                display: flex;
                flex-direction: row;
                justify-content: flex-start;
            ...
            <div
                ...
                `}
            >
                <Link to={path}><ChevronRight /></Link>
            </div>
        </PostCardContainer>
    )
}
